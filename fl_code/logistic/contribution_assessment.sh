...
for ((i = 0; i < $client_num; i++)); do
  ...
  nohup python logistic_server.py $port $(($client_num - 1)) >./log/lack_$i/server.log 2>&1 &
  if [ $? -eq 0 ]; then
    echo "submit logistic_server $port"
  else
    echo "submit logistic_server error" && exit
  fi
  ...
  for ((j = 0; j <= $(($client_num - 1)); j++)); do
    [ $j -eq $i ] && continue
    nohup python logistic_client.py $j $i >./log/lack_$i/client$j.log 2>&1 &
    if [ $? -eq 0 ]; then
      echo "submit logistic_client $j"
    else
      echo "submit logistic_client $j error" && exit
    fi
  done
