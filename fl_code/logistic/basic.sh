...
nohup python logistic_server.py $port $client_num >./log/all/server.log 2>&1 &
if [ $? -eq 0 ]; then
...
for ((i = 0; i <= $client_num - 1; i++)); do
  nohup python logistic_client.py $i -1 >./log/all/client$i.log 2>&1 &
  if [ $? -eq 0 ]; then
    echo "submit logistic_client $i"
  else
    echo "submit logistic_client $i error" && exit
  fi
done
